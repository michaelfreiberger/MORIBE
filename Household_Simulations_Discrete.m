function [HHDecision,HHEndowment] = Household_Simulations_Discrete(varargin)
...
if not(isfolder(strcat(SaveFolder,"\",Version,"\HHSimDiscrete\")))
    mkdir(strcat(SaveFolder,"\",Version,"\HHSimDiscrete\"))
end
...
function [Eprime,Aprime,Wprime,Pprime,Iprime,cprime,wprime,...
    ...
    for iteration = 1:MaxIterations
        ...
        if VFSTAY >= VFMOV
            ...
            cprime = TotalIncome - wprime2 - Aprime - pFactor * Eprime * Wprime * max(0,Pprime)^(1+phiPgrid(iphiP));
    ...
    for irho = 1:nrho
        for ia = 1:na
            for iphiP = 1:nphiP
                ...
                nquants = 20;
                Probquant = linspace(0.05,0.95,nquants);
                ...
                HHEndowment(1:nHH,2) = Agrid(unidrnd(nA,nHH,1));
                HHEndowment(1:nHH,3) = Wgrid(unidrnd(nW,nHH,1));
                ...
                while stop == 0 && iter <= MaxIter
                    ...
                    for jj = 1:5
                        quants(jj,:) = quantile(HHEndowment(:,jj),Probquant);
                        subplot(3,2,jj)
                        title(TitleVariables(jj))
                        plot(quants(jj,:),Probquant)
                        hold on
                    end
                    ...
                    if all(max(abs(quants-quantsOld),[],2) < [1e-3;Agrid(end)*1e-3;Wgrid(end)*1e-3;1e-3;1e-3])...
                            || iter >= MaxIter
                        stop = 1;
                        for jj = 1:5
                            subplot(3,2,jj)
                        ...
                        save(strcat(SaveFolder,"\",Version,"\HHSimDiscrete\Part_",num2str(iH0),"_",num2str(irho),"_",num2str(ia),"_",num2str(iphiP),".mat"),'Dec','Path','Characteristics','Policies','Grid','Para', '-v7.3')
                        break
                    else
                        iter = iter+1;
                        stop=0;
